# ☘️ St. Patrick's 3D Educational Adventure | Aventura Educativa 3D

[![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)]()
[![Three.js](https://img.shields.io/badge/Three.js-black?style=for-the-badge&logo=three.js&logoColor=white)]()
[![Vite](https://img.shields.io/badge/Vite-B73BFE?style=for-the-badge&logo=vite&logoColor=FFD62E)]()
[![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)]()

*(Scroll down for Spanish / Baja para la versión en español)*

## 🇬🇧 English

### About the Project
An interactive 3D educational web application designed for the Berta von Glumer School. It teaches students the history and myths of St. Patrick's Day through immersive storytelling and 5 dynamic minigames.

### Technical Highlights
Refactored from legacy code into a modern, scalable architecture featuring:
* **React & Vite:** Fast, modern front-end tooling.
* **Component Composition:** Highly modular UI broken down into specialized views and game components.
* **Custom Hooks & State Management:** Clean separation of game logic and 3D rendering.
* **Three.js Integration:** A custom-built, animated 3D Owl character that reacts to the player's choices without causing memory leaks.

### How to run locally
1. Clone the repository: `git clone https://github.com/MarcusPicazo/san-patricio-ebvg.git`
2. Install dependencies: `npm install`
3. Start the local server: `npm run dev`

---

## 🇪🇸 Español

### Sobre el Proyecto
Una aplicación web educativa e interactiva en 3D diseñada para la Escuela Berta von Glumer. Enseña a los alumnos la historia y los mitos del Día de San Patricio a través de una narrativa inmersiva y 5 minijuegos dinámicos.

### Aspectos Técnicos Destacados
Refactorizado desde código heredado hacia una arquitectura moderna y escalable que incluye:
* **React y Vite:** Herramientas de front-end rápidas y modernas.
* **Composición de Componentes:** Interfaz de usuario altamente modular dividida en vistas especializadas y componentes de juego.
* **Manejo de Estado Limpio:** Separación clara entre la lógica de los juegos y el renderizado 3D.
* **Integración con Three.js:** Un personaje Búho 3D animado y construido a medida que reacciona a las decisiones del jugador sin fugas de memoria.

### Cómo correrlo localmente
1. Clona el repositorio: `git clone https://github.com/MarcusPicazo/san-patricio-ebvg.git`
2. Instala las dependencias: `npm install`
3. Inicia el servidor local: `npm run dev`